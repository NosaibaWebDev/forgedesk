...
@section('title', 'עריכת חשבון')
@section('breadcrumbs')
<nav class="flex items-center gap-2 text-sm">
    <a href="{{ auth()->user()->isAdmin() ? route('admin.dashboard') : route('client.dashboard') }}" class="text-ink-muted hover:text-accent transition">בית</a>
    <i data-lucide="chevron-left" class="w-3.5 h-3.5 text-ink-muted"></i>
    <span class="text-ink font-medium">עריכת חשבון</span>
</nav>
@endsection
...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="bg-white rounded-card border border-border shadow-card p-6">
        <h2 class="text-lg font-semibold text-ink mb-5">תמונת פרופיל</h2>
            ...
            <div class="flex flex-col items-center gap-2">
                <div x-data="{ preview: '{{ $user->avatar && Storage::disk('public')->exists($user->avatar) ? Storage::disk('public')->url($user->avatar) : '' }}' }">
                    <template x-if="preview">
                        <img :src="preview" alt="Avatar" width="80" height="80" class="w-20 h-20 rounded-full object-cover border-2 border-border">
                    </template>
                    <template x-if="!preview">
                ...
                @if($user->avatar)
                    <form method="POST" action="{{ route('profile.avatar.destroy') }}" onsubmit="return confirm('האם אתה בטוח?')">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="text-xs text-danger hover:text-red-600 font-medium transition">הסר תמונה</button>
                    </form>
                @endif
            </div>
            <div class="flex-1">
                <form method="POST" action="{{ route('profile.update') }}" enctype="multipart/form-data" id="avatar-form">
                    ...
                    <div class="flex items-end gap-3">
                        <label class="flex-1">
                            <span class="label">בחר תמונה (מקסימום 2MB)</span>
    ...
    {{-- Personal Info --}}
    <div class="bg-white rounded-card border border-border shadow-card p-6">
        <h2 class="text-lg font-semibold text-ink mb-5">פרטים אישיים</h2>
    ...
    {{-- Password --}}
    <div class="bg-white rounded-card border border-border shadow-card p-6">
        <h2 class="text-lg font-semibold text-ink mb-5">שינוי סיסמה</h2>
        <form method="POST" action="{{ route('profile.password.update') }}">
            @csrf
            @method('PUT')
            <div class="space-y-4">
                <div>
                    <label class="label">סיסמה נוכחית</label>
                ...
                <div>
                    <label class="label">אישור סיסמה</label>
                    <input type="password" name="password_confirmation" required class="input" dir="ltr">
                </div>
                <button type="submit" class="btn-primary">עדכן סיסמה</button>
            </div>
        </form>
    </div>
</div>
