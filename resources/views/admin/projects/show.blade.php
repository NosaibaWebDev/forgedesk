...
<nav class="flex items-center gap-2 text-sm min-w-0">
    ...
    <a href="{{ route('admin.projects.index') }}" class="text-ink-muted hover:text-accent transition truncate">{{ __('projects') }}</a>
    <i data-lucide="chevron-left" class="w-3.5 h-3.5 text-ink-muted"></i>
    <span class="text-ink font-medium"><span class="hidden sm:inline">{{ $project->title }}</span><span class="sm:hidden">{{ \Illuminate\Support\Str::limit($project->title, 8) }}</span></span>
</nav>
@endsection
...
<div class="flex items-center gap-2 sm:gap-3">
    ...
    <a href="{{ route('admin.projects.edit', $project) }}" class="btn-secondary">
        ...
        <span class="hidden sm:inline">{{ __('edit_button') }}</span>
    </a>
...
<div class="space-y-4 sm:space-y-6">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-card border border-border shadow-card p-4 sm:p-6">
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6">
            <div>
                <p class="text-xs sm:text-sm text-ink-muted mb-1">{{ __('client') }}</p>
                <a href="{{ route('admin.clients.show', $project->user) }}" class="text-accent hover:underline font-medium text-sm truncate block">
                    {{ $project->user->name }}
                </a>
            </div>
            <div>
                <p class="text-xs sm:text-sm text-ink-muted mb-1">{{ __('status') }}</p>
                @php
                    $statusColors = [
                        ...
                        @foreach(\App\Enums\ProjectStatus::cases() as $s)
                            @if($s->value !== $project->status->value)
                                <button onclick="changeProjectStatus({{ $project->id }}, '{{ $s->value }}', this)"
                                        class="w-full text-left px-4 py-2 text-sm flex items-center gap-2 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition"
            ...
            <div>
                <p class="text-xs sm:text-sm text-ink-muted mb-1">{{ __('priority') }}</p>
                <span class="font-medium text-ink text-sm">{{ $project->priority_label }}</span>
            </div>
            <div>
                <p class="text-xs sm:text-sm text-ink-muted mb-1">{{ __('budget') }}</p>
                <span class="font-medium text-ink text-sm">₪{{ number_format($project->budget ?? 0, 0) }}</span>
            </div>
            <div>
                <p class="text-xs sm:text-sm text-ink-muted mb-1">{{ __('paid_amount') }}</p>
                <span class="font-medium text-ink text-sm">₪{{ number_format($project->paid_amount, 0) }}</span>
            </div>
            <div>
                <p class="text-xs sm:text-sm text-ink-muted mb-1">{{ __('progress') }}</p>
                <div class="flex items-center gap-2 mt-1">
                    ...
                    <span class="text-xs sm:text-sm font-medium text-ink">{{ $project->progress }}%</span>
                </div>
            </div>
            <div>
                <p class="text-xs sm:text-sm text-ink-muted mb-1">{{ __('hourly_rate') }}</p>
                <span class="font-medium text-ink text-sm">{{ $project->hourly_rate !== null ? '₪' . number_format($project->hourly_rate, 2) : '-' }}</span>
            </div>
            <div>
                <p class="text-xs sm:text-sm text-ink-muted mb-1">{{ __('estimated_hours') }}</p>
                <span class="font-medium text-ink text-sm">{{ $project->estimated_hours ?? '-' }}</span>
            </div>
            <div>
                <p class="text-xs sm:text-sm text-ink-muted mb-1">{{ __('estimated_total') }}</p>
                <span class="font-semibold text-accent text-sm">{{ $project->total_price !== null ? '₪' . number_format($project->total_price, 2) : '-' }}</span>
            </div>
            <div>
                <p class="text-xs sm:text-sm text-ink-muted mb-1">{{ __('start_date') }}</p>
                <span class="font-medium text-ink text-sm">{{ $project->start_date?->format('d/m/Y') ?? '-' }}</span>
            </div>
            <div>
                <p class="text-xs sm:text-sm text-ink-muted mb-1">{{ __('due_date') }}</p>
                <span class="font-medium text-ink text-sm">{{ $project->due_date?->format('d/m/Y') ?? '-' }}</span>
            </div>
            <div>
                <p class="text-xs sm:text-sm text-ink-muted mb-1">{{ __('completed_date') }}</p>
                <span class="font-medium text-ink text-sm">{{ $project->completed_at?->format('d/m/Y') ?? '-' }}</span>
            </div>
        </div>
        @if($project->description)
            <div class="mt-5 pt-5 border-t border-border">
                <p class="text-xs sm:text-sm text-ink-muted mb-1.5">{{ __('description') }}</p>
                <p class="text-ink-secondary text-sm">{{ $project->description }}</p>
            </div>
        @endif
        @if($project->notes)
            <div class="mt-5 pt-5 border-t border-border">
                <p class="text-xs sm:text-sm text-ink-muted mb-1.5">{{ __('notes') }}</p>
                <p class="text-ink-secondary text-sm">{{ $project->notes }}</p>
            </div>
        @endif
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-card border border-border shadow-card p-4 sm:p-6">
        <div class="flex items-center justify-between mb-5">
            ...
            <button onclick="document.getElementById('add-task-form').classList.toggle('hidden')" class="btn-primary">
                ...
                <span class="hidden sm:inline">{{ __('new_task') }}</span>
            </button>
        </div>

        <div id="add-task-form" class="hidden mb-6 p-4 sm:p-5 bg-surface rounded-card border border-border">
            <form method="POST" action="{{ route('admin.projects.tasks.store', $project) }}">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                        ...
                        <select id="task_status" name="status" required class="input">
                    ...
                    <div>
                        ...
                        <select id="task_priority" name="priority" required class="input">
                    ...
                    <div>
                        ...
                        <input type="number" id="task_estimated_hours" name="estimated_hours" min="0" step="0.5" class="input">
                    </div>
                    <div>
                        <label for="task_due_date" class="label">{{ __('due_date') }}</label>
                        <input type="date" id="task_due_date" name="due_date" class="input" style="max-width:305px">
                    </div>
                </div>
                <div class="mb-4">
                    <label for="task_description" class="label">{{ __('description') }}</label>
                    <textarea id="task_description" name="description" rows="2" class="input"></textarea>
                </div>
                <div class="flex gap-3">
                    <button type="submit" class="btn-primary">{{ __('add_task') }}</button>
                    <button type="button" onclick="document.getElementById('add-task-form').classList.add('hidden')" class="btn-ghost">{{ __('cancel') }}</button>
                </div>
            </form>
        </div>
            ...
            <table class="w-full text-right text-sm">
                <thead>
                    <tr class="border-b border-border">
                        <th class="px-3 sm:px-5 py-3 text-xs font-semibold text-ink-muted uppercase">{{ __('task_title') }}</th>
                        <th class="px-3 sm:px-5 py-3 text-xs font-semibold text-ink-muted uppercase hidden sm:table-cell">{{ __('status') }}</th>
                        <th class="px-3 sm:px-5 py-3 text-xs font-semibold text-ink-muted uppercase hidden lg:table-cell">{{ __('priority') }}</th>
                        <th class="px-3 sm:px-5 py-3 text-xs font-semibold text-ink-muted uppercase hidden md:table-cell">{{ __('assigned_to') }}</th>
                        <th class="px-3 sm:px-5 py-3 text-xs font-semibold text-ink-muted uppercase hidden lg:table-cell">{{ __('estimated_hours') }}</th>
                        <th class="px-3 sm:px-5 py-3 text-xs font-semibold text-ink-muted uppercase hidden lg:table-cell">{{ __('due_date') }}</th>
                        <th class="px-3 sm:px-5 py-3 text-xs font-semibold text-ink-muted uppercase">{{ __('actions') }}</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-border">
                    @forelse($project->tasks as $task)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition">
                            <td class="px-3 sm:px-5 py-3 font-medium text-ink">
                                <p class="truncate max-w-[140px] sm:max-w-none">{{ $task->title }}</p>
                                <div class="sm:hidden mt-1">
                                    ...
                                    @endphp
                                    <span class="rounded-badge px-2 py-0.5 text-[10px] font-medium ring-1 ring-inset {{ $taskStatusColors[$task->status->value] ?? '' }}">{{ $task->status_label }}</span>
                                </div>
                            </td>
                                        ...
                                        @foreach(\App\Enums\TaskStatus::cases() as $ts)
                                            @if($ts->value !== $task->status->value)
                                                <button onclick="changeTaskStatus({{ $project->id }}, {{ $task->id }}, '{{ $ts->value }}', this)"
                                                        class="w-full text-start px-3 py-1.5 text-xs flex items-center gap-2 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition"
                                                        style="color:var(--color-ink);">
                            ...
                            <td class="px-3 sm:px-5 py-3 text-ink-secondary hidden lg:table-cell">{{ $task->priority_label }}</td>
                            <td class="px-3 sm:px-5 py-3 text-ink-secondary hidden md:table-cell">{{ $task->assignee?->name ?? '-' }}</td>
                            <td class="px-3 sm:px-5 py-3 text-ink-secondary hidden lg:table-cell">{{ $task->estimated_hours ?? '-' }}</td>
                            <td class="px-3 sm:px-5 py-3 text-ink-secondary hidden lg:table-cell">{{ $task->due_date?->format('d/m/Y') ?? '-' }}</td>
                            <td class="px-3 sm:px-5 py-3">
                                <div class="flex items-center gap-2 sm:gap-3">
                                    <button onclick="toggleEditTask({{ $task->id }})" class="text-ink-secondary hover:text-ink text-xs sm:text-sm font-medium transition">{{ __('edit') }}</button>
                                    <button onclick="toggleTaskImages({{ $task->id }})" class="text-ink-secondary hover:text-ink text-xs sm:text-sm font-medium transition flex items-center gap-1">{{ __('task_images') }}</button>
                                    <form method="POST" action="{{ route('admin.projects.tasks.destroy', [$project, $task]) }}" onsubmit="return confirm('{{ __("confirm_delete_task") }}')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="text-danger hover:text-red-600 text-xs sm:text-sm font-medium transition">{{ __('delete') }}</button>
                                    </form>
                                </div>
                            </td>
                        </tr>
                        <tr id="task-images-{{ $task->id }}" class="hidden">
                            <td colspan="7" class="px-3 sm:px-5 py-4 bg-surface">
                                <div class="space-y-4">
                                        ...
                                        <div class="flex flex-wrap gap-2 sm:gap-3">
                                            @foreach($task->images as $image)
                                                <div class="relative group">
                                                    ...
                                                    <form method="POST" action="{{ route('admin.projects.tasks.images.destroy', [$project, $task, $image]) }}" onsubmit="return confirm('{{ __("confirm_delete_task") }}')" class="absolute top-1 start-1 opacity-0 group-hover:opacity-100 transition">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="w-5 h-5 sm:w-6 sm:h-6 rounded-full bg-danger text-white flex items-center justify-center text-xs shadow-sm hover:bg-red-600 transition">×</button>
                                                    </form>
                                                </div>
                                            @endforeach
                                        </div>
                                    @endif
                                    <form method="POST" action="{{ route('admin.projects.tasks.images.store', [$project, $task]) }}" enctype="multipart/form-data" class="flex flex-col sm:flex-row items-stretch sm:items-end gap-3">
                                        @csrf
                            ...
                            <td colspan="7" class="px-3 sm:px-5 py-4 bg-surface">
                                <form method="POST" action="{{ route('admin.projects.tasks.update', [$project, $task]) }}">
                                    ...
                                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-3">
                                        <input type="text" name="title" value="{{ $task->title }}" required class="input" placeholder="{{ __('task_title') }}">
                                        ...
                                        <input type="number" name="actual_hours" value="{{ $task->actual_hours }}" min="0" step="0.5" class="input" placeholder="{{ __('actual_hours') }}">
                                    </div>
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-3 sm:px-5 py-10 text-center">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-card border border-border shadow-card p-4 sm:p-6">
        ...
        <form method="POST" action="{{ route('admin.projects.files.store', $project) }}" enctype="multipart/form-data" class="mb-6 p-4 sm:p-5 bg-surface rounded-card border border-border">
            @csrf
            <div class="flex flex-col sm:flex-row items-stretch sm:items-end gap-3">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <input type="text" id="admin_file_notes" name="notes" class="input">
                </div>
                <button type="submit" class="btn-primary whitespace-nowrap self-end">{{ __('upload_files') }}</button>
            </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-right text-sm">
                <thead>
                    <tr class="border-b border-border">
                        <th class="px-3 sm:px-5 py-3 text-xs font-semibold text-ink-muted uppercase w-12"></th>
                        <th class="px-3 sm:px-5 py-3 text-xs font-semibold text-ink-muted uppercase">{{ __('file_name') }}</th>
                        <th class="px-3 sm:px-5 py-3 text-xs font-semibold text-ink-muted uppercase hidden sm:table-cell">{{ __('file_size') }}</th>
                        <th class="px-3 sm:px-5 py-3 text-xs font-semibold text-ink-muted uppercase hidden md:table-cell">{{ __('uploaded_by') }}</th>
                        <th class="px-3 sm:px-5 py-3 text-xs font-semibold text-ink-muted uppercase hidden lg:table-cell">{{ __('date') }}</th>
                        <th class="px-3 sm:px-5 py-3 text-xs font-semibold text-ink-muted uppercase">{{ __('actions') }}</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-border">
                    @forelse($project->files as $file)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition">
                            ...
                            <td class="px-3 sm:px-5 py-3 font-medium">
                                <a href="{{ $file->getTemporaryUrl() }}" target="_blank" class="text-accent hover:underline truncate block max-w-[180px] sm:max-w-none">{{ $file->original_name }}</a>
                            </td>
                            <td class="px-3 sm:px-5 py-3 text-ink-secondary hidden sm:table-cell">{{ $file->formatted_size }}</td>
                            <td class="px-3 sm:px-5 py-3 text-ink-secondary hidden md:table-cell">{{ $file->uploader->name ?? '-' }}</td>
                            <td class="px-3 sm:px-5 py-3 text-ink-secondary hidden lg:table-cell">{{ $file->created_at->format('d/m/Y H:i') }}</td>
                            <td class="px-3 sm:px-5 py-3">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-3 sm:px-5 py-10 text-center">
                                <div class="flex flex-col items-center gap-2">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-card border border-border shadow-card p-4 sm:p-6">
        ...
        <form method="POST" action="{{ route('admin.messages.store', $project) }}" class="mb-6">
            ...
            <div class="flex gap-2 sm:gap-3">
                <input type="text" name="body" required placeholder="{{ __('type_message') }}" class="input flex-1 min-w-0">
                <button type="submit" class="btn-primary flex-shrink-0 whitespace-nowrap">{{ __('send') }}</button>
            </div>
