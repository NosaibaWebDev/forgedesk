...
<div class="flex flex-wrap items-center gap-2 sm:gap-3">
    @if($client->projects->count())
        ...
        <a href="{{ route('admin.messages.show', $lastProject) }}" class="btn-ghost">
            ...
            <span class="hidden sm:inline">{{ __('send_message') }}</span>
        </a>
    @endif
...
<div class="space-y-4 sm:space-y-6">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-card border border-border shadow-card p-4 sm:p-6">
        <div class="flex flex-wrap items-center gap-3 sm:gap-4 mb-4 sm:mb-6">
            ...
            <div class="min-w-0">
                <p class="text-xl font-semibold text-ink truncate">{{ $client->name }}</p>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-card border border-border shadow-card">
        <div class="flex items-center justify-between p-4 sm:p-6 border-b border-border">
            ...
            <a href="{{ route('admin.projects.create') }}" class="btn-primary btn-sm inline-flex items-center gap-2">
                <i data-lucide="plus" class="w-4 h-4"></i>
                <span class="hidden sm:inline">{{ __('new_project_button') }}</span>
            </a>
        </div>
            ...
            <table class="w-full text-right">
                <thead>
                    <tr class="border-b border-border">
                        <th class="px-3 sm:px-6 py-3.5 text-xs font-semibold text-ink-muted uppercase tracking-wider">{{ __('project_name') }}</th>
                        <th class="px-3 sm:px-6 py-3.5 text-xs font-semibold text-ink-muted uppercase tracking-wider hidden sm:table-cell">{{ __('status') }}</th>
                        <th class="px-3 sm:px-6 py-3.5 text-xs font-semibold text-ink-muted uppercase tracking-wider hidden md:table-cell">{{ __('priority') }}</th>
                        <th class="px-3 sm:px-6 py-3.5 text-xs font-semibold text-ink-muted uppercase tracking-wider hidden sm:table-cell">{{ __('progress') }}</th>
                        <th class="px-3 sm:px-6 py-3.5 text-xs font-semibold text-ink-muted uppercase tracking-wider">{{ __('due_date') }}</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-border">
                    @forelse($client->projects as $project)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition">
                            ...
                            <td class="px-3 sm:px-6 py-3.5 hidden sm:table-cell">
                                @php
                                    $statusColors = [
                            ...
                            <td class="px-3 sm:px-6 py-3.5 text-sm text-ink-secondary hidden md:table-cell">{{ $project->priority_label }}</td>
                            <td class="px-3 sm:px-6 py-3.5 hidden sm:table-cell">
                                <div class="flex items-center gap-3">
                                    <div class="flex-1 bg-gray-100 dark:bg-gray-700 rounded-full h-2 w-20 sm:w-24">
                            ...
                            <td class="px-3 sm:px-6 py-3.5 text-sm text-ink-secondary">
