...
<div class="space-y-4 max-h-96 overflow-y-auto">
    @forelse($messages->sortBy('created_at') as $message)
        <div class="flex {{ $message->sender_id === $currentUserId ? 'justify-end' : 'justify-start' }}">
            <div class="max-w-md px-5 py-3.5 rounded-2xl {{ $message->sender_id === $currentUserId ? 'bg-accent/10 text-ink border border-accent/20' : 'bg-white dark:bg-gray-800 text-ink border border-border shadow-sm' }}">
