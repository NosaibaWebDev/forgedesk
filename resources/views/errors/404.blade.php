...
<head>
    ...
    <title>404 - {{ \App\Models\Setting::get('app_name', 'ForgeDesk Studio') }}</title>
...
<body class="min-h-screen flex items-center justify-center font-sans antialiased" style="background:var(--color-surface); color:var(--color-ink);">
    <div class="text-center px-4">
        <div class="w-20 h-20 rounded-card flex items-center justify-center mx-auto mb-6" style="background:var(--color-accent-light);">
            <span class="text-4xl font-bold text-accent">404</span>
        </div>
        <h1 class="text-3xl font-bold mb-2" style="color:var(--color-ink);">{{ __('page_not_found') }}</h1>
        <p class="text-lg mb-8" style="color:var(--color-ink-secondary);">{{ __('page_not_found_desc') }}</p>
